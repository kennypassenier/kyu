...
| Hub events | `curl "$HUB/t/mailbox.events/next?as=debug&envelope=json"` | what the hub decided and when |
| Logs | `docker logs mailbox` | startup, migrations, errors, refused logins |
...
|---|---|---|---|
| Poll returns `204` forever, publisher says `201` | The subscription was created **after** the message was published. It only sees what comes next (G7). | First poll carried a `mailbox-notice` header saying so. Dashboard shows the subscription with backlog 0 while the topic has messages. | Publish again, or `&from=beginning` to pull in what is retained. |
...
| Consumer sees nothing, siblings do | This subscription is **archived** (30 quiet days). | Dashboard shows state `archived`. `mailbox.events` has a `subscription.archived` for it. | `POST …/unarchive`, then raise its idle thresholds so it does not recur. The old backlog is gone — it lapsed, by design. |
| Messages appear then vanish | Retention collected them (7 days default) — but only ones no active or flagged subscription still needed. | `mailbox.events` has `message.expired`; `mailbox_messages` dropped. | Raise `retention_ms` for that topic, or `"never"`. |

## The same message keeps coming back

| Symptom | Likely cause | How you confirm | Fix |
|---|---|---|---|
| Redelivery every ~30 s | The consumer never acks, or crashes before acking. The lease expires and the message returns — that is at-least-once working. | `mailbox-attempt` header climbs. Dashboard shows it "in flight". | Ack after processing. If processing is genuinely slow, raise `lease_ms` for that subscription. |
| Attempts climb then stop, message gone | It ran out of attempts and was dead-lettered. | Dashboard's dead-letter section; `mailbox.events` has `message.dead_lettered`. | Fix the consumer, press **Requeue**. |
...
| Duplicates under one name | At-least-once. A lease expired mid-processing and the message was re-handed out. | `mailbox-attempt` > 1 on the duplicate. | Make the consumer idempotent. There is no exactly-once and there will not be (N4). |

## Messages hang rather than fail

| Symptom | Likely cause | How you confirm | Fix |
|---|---|---|---|
| Nothing redelivers, nothing dead-letters, backlog frozen | **The sweeper has stopped.** This is the failure that is invisible from the outside: without it, expired leases never return. | `curl -s $HUB/healthz` → `"sweeper":"stalled"`, HTTP 503. `mailbox_sweeper_age_ms` climbing. | Restart the container. Alert on `mailbox_sweeper_age_ms` so you learn this from monitoring rather than from a user. |
...
| Symptom | Likely cause | How you confirm | Fix |
|---|---|---|---|
| `503`, remedy mentions free space | The store cannot grow. mailbox refuses rather than confirming a publish it cannot keep. | `curl -s $HUB/healthz` → `"store":"unwritable"` with the error. `docker logs` has `a store write failed`. | Free space on the data volume, then check it is still mounted and writable. The hub recovers by itself once writes succeed. |
| `413` | Payload over `MAILBOX_MAX_BODY_BYTES` (1 MiB default). | The error names the limit. | Raise the limit, or put the bulk somewhere else and send a reference. mailbox is a message hub, not a file store. |
| `403`, "reserved for mailbox's own events" | You published to a `mailbox.*` topic. | The topic name starts with `mailbox.`. | Pick another name. You can still *subscribe* to `mailbox.*`. |
...
| Symptom | Likely cause | How you confirm | Fix |
|---|---|---|---|
| `401` on every call | The hub has a token and your call does not carry it. | The remedy shows the exact header. | `-H "authorization: Bearer $MAILBOX_TOKEN"`. The dashboard prints the whole command for you. |
...
| Hub refuses to start, names `MAILBOX_SECRET_KEY` | A token without a key. Half-configured is not allowed. | The error prints a freshly generated key to paste. | Paste it. Keep both values together. |
| Apps page lists tokens as `unreadable` | `MAILBOX_SECRET_KEY` changed. The old tokens cannot be decrypted. | The page says so in place. | Revoke each and generate replacements. See runbook §5. |
...
| Symptom | Likely cause | How you confirm | Fix |
|---|---|---|---|
| `cannot open the store at /data/mailbox.db` | The data directory is not writable by uid 65532. Almost always a bind mount, which does not inherit the image's ownership. | `docker logs` shows exactly that line with its remedy. | Use a named volume, or `chown 65532:65532` the host directory. See the comment in `compose.yml`. |
| Refuses to start, "newer schema" | The volume was written by a newer mailbox than this image. | The error says which versions. | Roll forward to the newer image, or restore the pre-migration snapshot beside the store. |
...
- **`mailbox.events` never fills up from its own housekeeping.** Retention
