...
| Field | Value |
|---|---|
| Current phase | **v3.5.0 released to GitHub 2026-09-20 20:30 UTC** (tag `fe8e3cf`, asset verified; `--check` reads and writes nothing — fix-check-1). NOT signed, NOT on CT 109 yet; **v3.4.0 LIVE** there since 19:49 UTC (merge-import proven by importing 0). Live W11 evidence: one `message.expired` at 19:32:20 UTC, expiries counted since |
...
| Next gate | Kenny-only: `scripts/sign-release.sh v3.5.0` from a plain terminal. Homelab Rust: the CT 109 deploy, which IS fix-check-1's measurement — on a copy of the schema-4 `data/kyu.db` first (`user_version` 4 before and after, no snapshot), then the live store ("store OK … (schema 5, current)"). Then fix-events-1 reads after 2026-09-21 19:35 UTC. Open form: relay of two kit findings to chassis-rs. Still open from before: chassis-rs's fix-3 follow-up; the `/api/clients` empty-array quirk |
| Next action | waiting on Kenny: `scripts/sign-release.sh v3.5.0` and the relay form |
| AFK mode | off |
...
| fix-check-1 · `--check` migrates the store | **RELEASED** as 3.5.0 (Klopt 2026-09-20): `--check` opens read-only, reports a pending migration, applies nothing; `tests/fix_check_1.rs` (real binary, red first). Measurement OPEN, restated 20:40 UTC because the live store is already at schema 5 and could not fail it: at the 3.5.0 deploy Homelab Rust runs `--check` as `kyu` on a COPY of the schema-4 `data/kyu.db` (with -wal/-shm) — pending migration reported, exit 0, `user_version` still 4, no `kyu.pre-v*.db` — then the live store as the weaker confirmation |
