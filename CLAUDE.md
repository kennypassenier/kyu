...
| Next gate | Kenny signs v3.2.0 (`scripts/sign-release.sh v3.2.0`); the deploy to CT 109 (2.4.1, still rolled back) is the Homelab Rust session's, waiting on the signed musl release before retrying (V6: binary to `/opt/kyu/bin`, the kit unit, `KYU_DATA_DIR` → `KYU_STATE_DIR` in `kyu.env`). chassis-rs is waiting on one line back once the supervised `update --check` runs on CT 109: whether it read the state directory from the unit or the built-in default |
| AFK mode | off |
