...
| Field | Value |
|---|---|
| Current phase | **v3.2.0 released to GitHub 2026-09-10** (tag `v3.2.0` = `85678b8`, release-go given at the report form): chassis-rs 2.0.0 adopted (`Client::adopted` replaces a field-by-field literal, `Client` is `#[non_exhaustive]` now). `chassis sync --write` picked up the musl release pipeline kyu proposed after 3.1.1 — `Dockerfile`/`release.yml`/`rust-toolchain.toml` are fully scaffold-owned again — and fixed `deploy/service.yml`'s `update_cmd` (fix-3: two missing `--property=Environment=` lines that made a supervised `update --check` miss `KYU_STATE_DIR`; chassis-rs confirmed kyu is the expected first project to measure this live). `tests/common`'s manual login workaround is gone (CF-12 fixed upstream, confirmed closed by chassis-rs). 184 tests green, container-smoke green, release workflow green (5m49s). The downloaded asset is verified: `SHA256SUMS` matches, `ldd` reports no dynamic links, `kyu --version` prints `3.2.0`. `.githooks/commit-msg`/`.claude/hooks/check-commit.sh` carry dev-procedure's ID-gate logic that `chassis sync` does not know about and will keep proposing to overwrite — shown to Kenny as a live diff at the release-go gate, accepted as a permanent risk with no fix available from kyu's side |
| Last completed gate | Mini-round (2026-09-09/10): chassis-rs 1.8.0 upgrade + five adoptions (3.1.0) → the glibc/musl fix after the failed CT 109 deploy (3.1.1) → chassis-rs 2.0.0 adoption, released (3.2.0) |
| Next gate | v3.2.0 signed (2026-09-10, `SHA256SUMS.minisig` + `VERSION` on the release) and deployed to CT 109 the same day — reported by the Homelab Rust session, not independently verified from here: `/healthz` ok, store writable, sweeper alive, `NRestarts` 0, all V6 steps done, `deploy/service.yml`'s `update_cmd` (fix-3) in the running unit, Kenny approved the guarded nightly update. That update has not fired yet, so chassis-rs's one requested line (whether the supervised `update --check` read the state directory from the unit) is still open — follow up once it runs. Also relayed to chassis-rs: `GET /api/clients` answers with an empty array for a few minutes right after restart before a just-imported app appears, observed live on CT 109 — likely a kit-level (chassis-rs) load-order quirk, not kyu's own code |
| AFK mode | off |
