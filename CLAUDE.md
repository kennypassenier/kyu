...
| Field | Value |
|---|---|
| Current phase | **v3.2.0 on `main`, gates green, awaiting Kenny's release-go** (2026-09-10, commit `459d1c0`): chassis-rs 2.0.0 adopted (`Client::adopted` replaces a field-by-field literal, `Client` is `#[non_exhaustive]` now). `chassis sync --write` picked up the musl release pipeline kyu proposed after 3.1.1 — `Dockerfile`/`release.yml`/`rust-toolchain.toml` are fully scaffold-owned again — and fixed `deploy/service.yml`'s `update_cmd` (fix-3: two missing `--property=Environment=` lines that made a supervised `update --check` miss `KYU_STATE_DIR`). `tests/common`'s manual login workaround is gone (CF-12 fixed upstream). 184 tests green, container-smoke green. `.githooks/commit-msg`/`.claude/hooks/check-commit.sh` carry dev-procedure's ID-gate logic that `chassis sync` does not know about and will keep proposing to overwrite — restored by hand this round, no permanent fix available from kyu's side |
| Last completed gate | Mini-round (2026-09-09/10): chassis-rs 1.8.0 upgrade + five adoptions (3.1.0) → the glibc/musl fix after the failed CT 109 deploy (3.1.1) → chassis-rs 2.0.0 adoption (3.2.0) |
| Next gate | Kenny signs v3.2.0 (`scripts/sign-release.sh v3.2.0`); the deploy to CT 109 (2.4.1, still rolled back) is the Homelab Rust session's, waiting on the signed musl release before retrying (V6: binary to `/opt/kyu/bin`, the kit unit, `KYU_DATA_DIR` → `KYU_STATE_DIR` in `kyu.env`) |
| AFK mode | off |
