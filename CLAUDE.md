...
| Field | Value |
|---|---|
| Current phase | **v3.1.1 released to GitHub 2026-09-09** (tag `v3.1.1` = `0f004c7`): v3.1.0's release binary did not run on CT 109 (Homelab Rust's deploy failed with `GLIBC_2.39' not found`, rolled back within 90s, no damage) — built against Debian trixie's glibc, CT 109 has Debian 12's. Fixed by restoring T9 (frozen at Phase 3, silently dropped by the chassis-rs 3.0.0 migration's scaffold Dockerfile): the release binary and container image are static musl again (`gcr.io/distroless/static:nonroot`, uid 65532), and the release workflow now fails the build if that ever regresses (`ldd` check, red-then-green proven). Verified: the actual downloaded v3.1.1 asset is `ldd`-static and runs. Still carries the chassis-rs 1.8.0 upgrade + five additive adoptions from the same day (K-vocabulary, K-harness, K-cli, K-kitdocs, K-actions/W17 "Prune every dead letter") — see the queue row below. CT 109 is on 2.4.1 (rolled back, confirmed by Homelab Rust) |
| Last completed gate | Mini-round (2026-09-09): chassis-rs 1.8.0 upgrade + five additive adoptions (3.1.0), then the glibc/musl fix after the failed CT 109 deploy (3.1.1) |
| Next gate | Kenny signs v3.1.1 (`scripts/sign-release.sh v3.1.1`); the deploy to CT 109 (2.4.1) is the Homelab Rust session's, which parked all three services' `release_repo` after the failed attempt and is waiting on this fix before retrying (V6: binary to `/opt/kyu/bin`, the kit unit, `KYU_DATA_DIR` → `KYU_STATE_DIR` in `kyu.env`) |
| AFK mode | off |
