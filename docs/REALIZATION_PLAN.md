...
| Mini-round · W11 expiry window | 2026-09-18 | Found from the Home Assistant side: 15,690 queue items "kyu: berichten verlopen", traced to 27,991 `message.expired` events (27,969 with count 1) for one unpolled subscription between 2026-09-04 and 2026-09-10, looping through HA back onto the same topic. Kenny chose: one event per subscription per 24 h, counted (over 1 h, over silencing unpolled subscriptions, over an HA-only throttle); plus a 24 h per-topic throttle in `automation.hub_kyu_events_webhook`; the lost client tokens handed to the Homelab Rust session. Two correction records opened (fix-events-1, fix-state-1), the second for the empty store the 3.x unit opened on CT 109 | `docs/CORRECTIONS.md`, FEATURES.md W11 amendment, AR6 amendment, migration 5, 3.3.0 |
| Mini-round · W11 report + corrections | 2026-09-18 | Report on the hub window (Akkoord) and on the HA throttle (Akkoord); correction records fix-events-1 and fix-state-1 both Klopt, their measurements stay open; release of 3.3.0 → **Later** (not tagged). Found the same evening, verified from both sessions: the restored store did not restore the door — kyu 3.x reads `clients.json.enc`, the one-time import of the 2.x `apps` table never ran because the file already existed; 2 of 8 live apps can authenticate. The door question and the `KYU_DATA_DIR` line in CT 109's `kyu.env` are in a second open form | `docs/CORRECTIONS.md`, CLAUDE.md |
| Release 3.3.0 | 2026-09-20 | Kenny's go on the report form ("Claude tagt v3.3.0 na groene CI, de workflow publiceert"), after the door was refilled and `KYU_DATA_DIR` had left CT 109's `kyu.env`. Tagged `8af266b` (CI green), release workflow green in 4 min 25 s, asset verified: `sha256sum -c SHA256SUMS` OK, `kyu --version` → 3.3.0, `ldd` → statically linked. Signing (`scripts/sign-release.sh v3.3.0`) is Kenny's; the CT 109 deploy with the fix-state-1 drill is Homelab Rust's | tag `v3.3.0`, GitHub release |
| Mini-round · merge-import | 2026-09-20 | Kenny: "ja, merge-import in 3.3.0" — 3.3.0 was already tagged on his earlier go, so it ships as 3.4.0 with the same "tag after green CI" go. Import of the 2.x `apps` table at every start, by name; two tests red first (`left: 0, right: 1`) | `src/kit.rs`, `tests/door_import.rs`, FEATURES.md W2 amendment, CHANGELOG 3.4.0 |
| Release 3.4.0 | 2026-09-20 | Kenny's "tag na groene CI" go applied to the version carrying the merge-import. Tagged `e03a2a7` (CI green, all four jobs), release workflow green in 5 min 38 s, asset verified from the download. Signing is Kenny's; the CT 109 deploy with the fix-state-1 drill is Homelab Rust's | tag `v3.4.0`, GitHub release |
| Correction · fix-check-1 | 2026-09-20 | Klopt: `--check` opens the store read-only and applies nothing (found by Homelab Rust at the 3.3.0 drill). Built test-first on the real binary; ships as 3.5.0, release go pending | `src/store/mod.rs` (`inspect`), `src/main.rs`, `tests/fix_check_1.rs`, CHANGELOG 3.5.0 |
| Deploy 3.4.0 | 2026-09-20 | Homelab Rust, 19:49 UTC, after Kenny's signature at 19:48; drill run on a copy as the `kyu` user (live store untouched); the merge-import reported 0 — all eight apps already in the door, so the manual merge of step-10 was complete. Verified from this session | CT 109, kyu's CLAUDE.md |
| Release 3.5.0 | 2026-09-20 | Kenny's go ("Claude tagt v3.5.0 na groene CI"). Tagged `fe8e3cf` (CI green, four jobs), release workflow green in 6 min 05 s, asset verified from the download (SHA256SUMS OK, `--version` 3.5.0, statically linked); the release binary's `--check` on a fresh state dir prints "no store yet … created at start", exits 0 and creates nothing. Signing is Kenny's; the CT 109 deploy is Homelab Rust's and doubles as fix-check-1's measurement | tag `v3.5.0`, GitHub release |
| Deploy 3.5.0 · fix-check-1 measured | 2026-09-20 | Homelab Rust, 20:41 UTC after Kenny's signature. The measurement that could fail did not: `--check` as `kyu` on a copy of the schema-4 `data/kyu.db` left `user_version` 4, no snapshot, exit 0 naming the pending migration; guard fix-state-1 re-drilled; live store untouched to the nanosecond. Verified from this session | CT 109, CORRECTIONS.md fix-check-1 |
