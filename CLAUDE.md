...
| Next gate | Kenny-only: `scripts/sign-release.sh v3.5.0` from a plain terminal. Homelab Rust: the CT 109 deploy — and that deploy IS fix-check-1's measurement: `--check` against the live store must print "store OK … (schema 5, current)" and leave no `kyu.pre-v*.db`. Then fix-events-1 reads after 2026-09-21 19:35 UTC. Open form: relay of two kit findings to chassis-rs (the ReadWritePaths remedy on a root-owned binary; a check that writes — the second now fixed on kyu's side). Still open from before: chassis-rs's fix-3 follow-up; the `/api/clients` empty-array quirk |
...
| fix-check-1 · `--check` migrates the store | **BUILT** 2026-09-20 (Klopt): `--check` opens read-only, reports a pending migration, applies nothing; `tests/fix_check_1.rs` (real binary, red first). Ships as 3.5.0. Measurement OPEN: first CT 109 deploy carrying it — `--check` against the live store leaves no `kyu.pre-v*.db` |
