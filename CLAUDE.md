...
| Next gate | Door CLOSED by hand on 2026-09-20 (Homelab Rust; verified here: 8 clients in `clients.json.enc`, zero 401 since 16:30 UTC) and `KYU_DATA_DIR` gone from CT 109's `kyu.env` since 2026-09-19 — both blockers of the 3.3.0 deploy are gone. Open form (2026-09-20): does kyu still gain an idempotent merge-import (name-based, see CORRECTIONS.md fix-state-1 follow-up), and the release go for 3.3.0 now that nothing blocks it. Still open from 3.2.1: never signed/deployed; chassis-rs's fix-3 follow-up; the `/api/clients` empty-array quirk |
| Next action | waiting on Kenny: the merge-import / release form of 2026-09-20 |
| AFK mode | off |
...
| fix-state-1 · measurement | **OPEN, restated 2026-09-20** — `KYU_DATA_DIR` is already gone from `kyu.env` (2026-09-19) and the old store is back (2026-09-18), so the drill at the 3.3.0 deploy is deliberate: `kyu --check` once with `KYU_DATA_DIR=…/data` added to the unit environment must refuse naming both directories, then the normal `--check` passes. Homelab Rust runs the deploy |
