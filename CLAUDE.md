...
| Field | Value |
|---|---|
| Current phase | **v3.2.1 released to GitHub 2026-09-10** (tag `v3.2.1` = `b849738`): chassis-rs 2.0.0 → 2.0.2, a kit-only bump — neither 2.0.1 nor 2.0.2 touches the public API (`app.rs` byte-identical across the range); `docs/KIT.md` regenerated. Found and fixed while upgrading: `tests/p7_cli.rs`'s `run()` helper read a spawned child's stdout/stderr only after it exited, which deadlocks once the child writes more than this sandbox's pipe buffer before anyone reads it — `--help`'s own 8.4 KB now does exactly that. Two reader threads drain both pipes concurrently with the wait loop instead; not a chassis-rs regression. Gates green, release workflow green (5m35s), asset verified (`SHA256SUMS`, `ldd`, `--version`) |
| Last completed gate | Mini-round (2026-09-09/10): chassis-rs 1.8.0 upgrade + five adoptions (3.1.0) → the glibc/musl fix after the failed CT 109 deploy (3.1.1) → chassis-rs 2.0.0 adoption, released (3.2.0) → chassis-rs 2.0.2 bump, released (3.2.1) |
| Next gate | v3.2.1 needs Kenny's `scripts/sign-release.sh v3.2.1` and a deploy to CT 109 before it does anything for production; v3.2.0 IS live there (confirmed 2026-09-10 by Homelab Rust: `/healthz` ok, store writable, sweeper alive, all V6 steps done, fix-3's `update_cmd` in the running unit). The guarded nightly update Kenny approved had not fired as of the 3.2.0 round, so chassis-rs's one requested line (whether the supervised `update --check` read the state directory from the unit, fix-3) is still open — follow up once it runs, on whichever version is live by then. Also relayed to chassis-rs and not yet followed up: `GET /api/clients` answers with an empty array for a few minutes right after restart before a just-imported app appears, observed live on CT 109 — likely a kit-level (chassis-rs) load-order quirk, not kyu's own code |
| AFK mode | off |
