...
| Field | Value |
|---|---|
| Current phase | **v3.4.0 LIVE on CT 109 since 2026-09-20 19:49:58 UTC** (verified: `/healthz` 3.4.0 ok, the merge-import reported 0 — all 8 apps were already in the door, which also proves the manual merge complete; zero 401). **3.5.0 built, release go pending** (fix-check-1: `--check` reads and writes nothing). Live evidence for the W11 window so far: one `message.expired` at 19:32:20 UTC for `notify.kenny`/`desktop`, `expired_unannounced` 2 since — counting, not announcing |
...
| Next action | waiting on Kenny: the release go for 3.5.0 and the relay form (chassis-rs findings) |
| AFK mode | off |
