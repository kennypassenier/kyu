...
| Next action | waiting on Kenny: the relay form (chassis-rs findings), the fix-check-1 correction form, and `scripts/sign-release.sh v3.4.0` from a plain terminal (the first attempt got an empty password in the panel) |
| AFK mode | off |
...
| fix-check-1 · `--check` migrates the store | **OPEN** (found 2026-09-20 at the 3.3.0 drill; correction form with Kenny) — proposed: `--check` opens read-only, reports a pending migration, applies nothing; test = real binary with `--check` on a version-4 store leaves `user_version` 4 and no `kyu.pre-v*.db`. Record in CORRECTIONS.md |
